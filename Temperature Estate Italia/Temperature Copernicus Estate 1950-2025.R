...
for (start in start_years) {
  ...
  for (mese in c("06","07","08")) {
    req = list(
      ...
      "area" = c(47, 6, 36, 19),
      "format" = "netcdf",
      "target" = paste0("ERA5L_T2m_Mean_Italy_", mese, "", start, "", end, ".nc")
    )
...
## Risolvi evenduali duplicati

setDT(df_all)

df_unique <- df_all[, .(t2m = mean(t2m)), by = .(lon, lat, date)]


## Controlla se ci sono tutti gli anni/mesi
...
df_unique[points_with_regions, on = .(lon, lat), region := i.region]

# aggiungiamo colonne temporali direttamente in df_unique
df_unique[, year  := year(date)]
df_unique[, month := month(date)]
df_unique[, day   := yday(date)]

# normalizziamo i nomi delle regioni direttamente
df_unique[region == "Apulia", region := "Puglia"]
df_unique[region == "Sicily", region := "Sicilia"]

# se ti bastano solo i record con regione assegnata
df_unique <- df_unique[!is.na(region)]

df_labeled <- df_unique


write.csv(df_labeled, file="Output/all_data.csv", row.names = F)
...
df_labeled %>%
  filter(year %in% c(1961:1990, 1991:2020, 2025)) %>%
...
italy_average_daily =  df_labeled %>%
  ...
  fake_date = as.Date(sprintf("2025-%02d-%02d", month, day))) %>%
  group_by(periodo, fake_date) %>%
