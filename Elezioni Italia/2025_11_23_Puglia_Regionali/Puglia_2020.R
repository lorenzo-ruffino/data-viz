...
setwd("~/Documents/Progetti/data-viz/Elezioni Italia/2025_11_23_Puglia_Regionali")
...
coordinate = read_csv("puglia_centroidi.csv")
...
data_grouped = data %>%
  ...
  mutate(R = case_when(
    COGNOME == "FITTO" ~ "CDX",
    COGNOME %in% c("EMILIANO", "LARICCHIA") ~ "CSX/M5S",
    TRUE ~ "Altri"
  )) %>%
...
write.csv(risultati, "risultati_puglia_regionali.csv", row.names = FALSE)
