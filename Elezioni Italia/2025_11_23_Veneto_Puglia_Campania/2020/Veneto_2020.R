...
setwd("~/Documents/Progetti/data-viz/Elezioni Italia/2025_11_23_Veneto_Regionali")

data = read_excel("Regionali_AltreRegioni_Scrutini.xlsx")

coordinate = read_csv("veneto_coordinate.csv")
...
data_grouped = data %>%
  ...
  mutate(R = case_when(
    COGNOME == "ZAIA" ~ "CDX",
    COGNOME %in% c("LORENZONI", "CAPPELLETTI") ~ "CSX/M5S",
    TRUE ~ "Altri"
  )) %>%
...
risultati = data_pivot %>%
  ...
  mutate(
    ...
    DIFF_VOTI = VOTI_CDX - `VOTI_CSX/M5S`,
    COMUNE_CLEAN = gsub("[^A-Z0-9]", "", stri_trans_general(toupper(COMUNE), "Latin-ASCII"))
  ) %>%
  left_join(coordinate_clean, by = "COMUNE_CLEAN")%>%
  select(-COMUNE_CLEAN)

write.csv(risultati, "risultati_veneto_regionali.csv", row.names = FALSE)
